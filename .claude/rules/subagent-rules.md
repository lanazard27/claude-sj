...
### BLOCKED 에스컬레이션 보고서
사용자에게 보고 시 아래 구조로 전달:
- **문제**: 에이전트가 막힌 지점
- **시도**: 해보았던 방법들
- **필요 결정**: 사용자가 선택해야 할 옵션
- **sj 제안**: 대안 방안
...
### 위임 프롬프트 템플릿 (필수 사용)
에이전트 스폰 시 **반드시** `rules/delegation-templates.md`의 해당 템플릿 사용. 템플릿 없이 자유 형식 위임 금지. **Verbatim 복사 원칙**은 delegation-templates.md 공통 규칙 참조.
