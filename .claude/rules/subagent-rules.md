# Sub-agent 위임 규칙
<!-- 목차: 라우팅(3줄) | 복잡도라우팅(14줄) | 모델승격(26줄) | 위임패턴(51줄) | 상태관리(174줄) | CONCERNS분류(195줄) | 질문처리(233줄) | 의존성설치(244줄) | DB스키마(251줄) | 위임프롬프트(261줄) | 품질체크리스트(313줄) | 검증루프(321줄) -->
...
**분할 사전 검증 (sj가 architect 출력 후 실행):**
architect가 스펙 분할안을 제시하면, 병렬 진행 전에 sj가 아래 체크리스트로 검증:

```
architect 분할안 수령
    ↓
sj 체크리스트:
    1. 파일 중복 없음? → 각 스펙의 파일 목록을 비교. 교집합 있으면 Phase 0/2로 이동
    2. 숨은 의존성 없음? → 스펙 A가 스펙 B의 타입/함수를 import하면 의존성 존재
    3. 각 스펙 3-6개 파일? → 7개+면 분할 요청. 1-2개면 다른 스펙과 병합 검토
    4. Phase 2 연결 복잡도? → 연결 파일이 10개+면 분할이 비효율적. 순차 전환 고려
    ↓
    ├── 통과 → 병렬 진행
    └── 실패 → architect에 수정 요청 또는 순차로 전환
```
...
## 에이전트 상태 관리 (Superpowers 방식)

모든 에이전트는 작업 완료 후 반드시 아래 **상태 중 하나**를 반환:

| 상태 | 의미 | sj 대응 |
|------|------|---------|
| **DONE** | 작업 완료, 이슈 없음 | 다음 단계 진행 |
| **DONE_WITH_CONCERNS** | 완료했지만 우려사항 있음 | 우려 내용 검토 후 진행. 정확성/범위 문제면 수정. 관찰 수준이면 기록 후 진행 |
| **NEEDS_CONTEXT** | 정보 부족으로 작업 불가 | sj가 누락 정보를 판단/제공 후 새 스폰 |
| **BLOCKED** | 근본적 차단 | 원인 분석 → 컨텍스트 보충 / 모델 승격 / 태스크 분할 / 사용자 에스컬레이션 |

**BLOCKED 처리 우선순위:**
1. 컨텍스트 문제 → 더 많은 정보 제공 후 새 스폰
2. 능력 부족 → 상위 모델로 승격 후 새 스폰
3. 태스크 과대 → 작게 분할 후 재스폰
4. 설계 자체 오류 → 사용자에게 에스컬레이션

**절대 하지 않을 것:**
- 같은 모델/같은 프롬프트로 재시도 (상황이 안 바뀌면 결과도 같음)
- BLOCKED 무시하고 강제 진행

### DONE_WITH_CONCERNS 자동 분류 기준

에이전트가 DONE_WITH_CONCERNS를 반환하면, sj가 concerns 내용을 기준으로 **자동 분류**:

| 카테고리 | 판단 기준 | sj 대응 |
|---------|----------|---------|
| **진행 가능** (안전) | "이 파일이 좀 길어졌음", "나중에 리팩토링하면 좋겠음", "성능 최적화 여지 있음" | 기록 후 바로 다음 단계 진행 |
| **검토 필요** (주의) | "기존 코드와 패턴이 좀 다름", "에러 처리는 했는데 확신 없음", "특정 케이스는 테스트 못 함" | sj가 해당 부분만 빠르게 Read로 확인 후 진행 |
| **수정 필요** (위험) | "요구사항 일부만 구현됨", "에러 시나리오 누락 있음", "보안 검토 필요" | FAIL과 동일 취급. code-writer에게 수정 지시 |

**분류 원칙:**
- 애매하면 **한 단계 아래로** (안전 → 주의, 주의 → 위험)
- 사용자에게는 분류 결과만 간단히 보고 ("이슈 있어서 수정 지시함" 또는 "괜찮아서 진행")
...
3. **제약**: 한국어, 범위 밖 수정 금지, 작업 시작 전 꼭 필요한 정보가 부족하면 질문 가능 (단, 기술 스택/아키텍처 결정은 sj가 이미 함)
4. **출력**: 반환 형식 지정 + **상태 반환 필수** (DONE / DONE_WITH_CONCERNS / NEEDS_CONTEXT / BLOCKED)
...
## 에이전트 질문 처리 (제한적 허용 — Superpowers 방식)
...
**질문 허용 범위:**
- **허용**: 파일 경로, 스키마 필드명, 기존 패턴 위치, API 엔드포인트 형식 등 구체적 정보
- **허용 안 함**: 기술 스택 선택, 아키텍처 결정, 라이브러리 버전 — 이건 sj가 이미 결정해서 프롬프트에 포함

**sj의 대응:**
1. 에이전트 질문을 읽고 **sj가 직접 답변** (사용자에게 전달하지 않음)
2. 답변 근거: 이전 경험, memory, 프로젝트 컨텍스트, 안정성 우선
3. 답변을 포함해 **새 에이전트 스폰** (resume 금지)
4. 사용자에게는 "에이전트가 X에 대해 질문해서 Y라고 답변했어" 정도로 요약

**resume 사용 금지 (유지):**
- 질문 처리 시에도 resume 대신 새 스폰
- resume는 이전 컨텍스트 전체 복원으로 혼선 위험
