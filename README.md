...
| 훅 | 역할 |
|---|------|
| `audit-log.sh` | 도구 사용 이력 기록 + 세션별 통계 수집 (통합) |

### Lifecycle 훅

| 훅 | 역할 |
|---|------|
| `session-start.sh` | 세션 시작 시 Git 상태, 통계, 메모리 건강 체크 |
| `on-stop.sh` | 메모리 크기 체크 및 경고 |
| `on-session-end.sh` | 세션 종료 시 메모리 최적화 |
| `on-subagent-stop.sh` | 하위 에이전트 종료 시 결과 검증 리마인드 |
...
| 🎬 미디어 | 영상, 이미지 분석 |
| 🔍 문제해결 | 디버깅, 트러블슈팅 |
| 🔥 핫픽스 | 긴급 수정, production 버그 |
| 🔌 스킬 | 웹사이트 클론, 디자인 복제 |
| 📋 프로젝트 리딩 | 기존 프로젝트 문서 정리 + 에이전트 최적화 전달 |

### 개발 모드 흐름

```
규모 질문 (소형/중형/대규모) → Confidence Check → PRD → 사용자 승인 → 구현 → 검증
```
...
| 규모 | 기준 | 워크플로우 |
|------|------|-----------|
| 소형 | 1-2파일, 단순 변경 | 미니 PRD → 바로 구현 |
| 중형 | 3-5파일, 신규 기능 | 스탠다드 PRD → Phase B~G |
| 대규모 | 5파일+, 다중 기능 | 문서 8개 순차 제작 (질문→정리→정합성) → Phase C~G |

**대규모 문서 제작 순서:** PRD → 비즈니스 규칙 → 아키텍처 → RBAC → 마일스톤 → 기능별 스펙 → UI 목업 → API/에러코드

### 프로젝트 리딩 모드

기존 프로젝트(문서가 이미 준비된 경우)를 가져와서 개발할 때:

```
문서 파악 → 정합성 검증 → 마일스톤별 브리프 작성 → 에이전트 전달
```

**마일스톤 브리프**: 전체 문서를 덤프하지 않고, 해당 마일스톤에 필요한 섹션만 발췌·교차참조 해결·구조화하여 에이전트에게 전달. "아키텍처가 이해하기 쉽게 전해야 좋은 결과물이 나온다"는 원칙.
...
│   ├── audit-log.sh       # 감사 로그 + 통계 (통합)
│   ├── session-start.sh
│   ├── on-stop.sh
│   ├── on-session-end.sh
│   └── on-subagent-stop.sh
├── rules/                 # 상세 규칙 (필요시만 읽기, 자동 로딩 안 함)
│   ├── modes.md           # 모드 감지(9개) + 개발 워크플로우 + 규모 분기 + 프로젝트 리딩 + 마일스톤 브리프
│   ├── subagent-rules.md  # 에이전트 위임 + Token Budget + 도메인 에이전트
│   ├── delegation-templates.md # 에이전트 위임 프롬프트 템플릿
│   ├── maintenance.md     # 메모리 관리 + Reflexion Memory
│   └── context-template.md # 프로젝트 컨텍스트 템플릿
├── sessions/              # 세션 상태 저장
│   └── latest.md          # 자동 저장 (에이전트 완료 시)
├── memory/                # 세션 간 지식
...
- **rules/*.md**: 상세 규칙 (CLAUDE.md에서 상황별로 필요시 Read)
